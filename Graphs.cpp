...
CharacterGraphs::CharacterGraphs() {
	...
	for (int i = 0; i < 5; i++) {
		...
		m_specialCharacterGraphs.push_back(-1);
	}
	...
	commonPath = "picture/special/";
	m_specialCharacterGraphs[0] = LoadGraph((commonPath + "アカツキ.png").c_str());
	m_specialCharacterGraphs[1] = LoadGraph((commonPath + "トウノ.png").c_str());
	m_specialCharacterGraphs[2] = LoadGraph((commonPath + "タキノ.png").c_str());
	m_specialCharacterGraphs[3] = LoadGraph((commonPath + "カイバラ.png").c_str());
	m_specialCharacterGraphs[4] = LoadGraph((commonPath + "カンエイ.png").c_str());
	...
	m_statusGraphs[4] = LoadGraph((commonPath + "防御力上昇.png").c_str());
	m_statusGraphs[5] = LoadGraph((commonPath + "防御力下降.png").c_str());
}
...
int CharacterGraphs::getSpecialCharacterGraphs(CHARACTER_NUM characterNum) const {
	return m_specialCharacterGraphs[characterNum];
}
...
EffectGraphs::EffectGraphs() {
	for (int i = 0; i < getEffectSize(ATARI_EFFECT); i++) {
		ostringstream oss;
		oss << "picture/effect/orange" << i + 1 << ".png";
		m_atariGraphs.push_back(LoadGraph(oss.str().c_str()));
	}
	for (int i = 0; i < getEffectSize(SKILL_EFFECT); i++) {
		ostringstream oss;
		oss << "picture/effect/skill" << i + 1 << ".png";
		m_skillGraphs.push_back(LoadGraph(oss.str().c_str()));
	}
}
...
EffectGraphs::~EffectGraphs() {
	...
	for (unsigned i = 0; i < m_skillGraphs.size(); i++) {
		DeleteGraph(m_skillGraphs[i]);
	}
}
